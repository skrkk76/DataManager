...
public class ProductivityGraph extends RDMServicesConstants {
    ...
    public String loadProductivityGraph(String sUserId, String sFName, String sLName, String sDept, String sStartDt,
	    String sEndDt, StringList slUsers) throws Exception {
	return getProductivityGraphCSV(sUserId, sFName, sLName, sDept, sStartDt, sEndDt, slUsers, false);
    }

    public String exportProductivityGraph(String sUserId, String sFName, String sLName, String sDept, String sStartDt,
	    String sEndDt) throws Exception {
	return getProductivityGraphCSV(sUserId, sFName, sLName, sDept, sStartDt, sEndDt, null, true);
    }

    private String getProductivityGraphCSV(String sUserId, String sFName, String sLName, String sDept, String sStartDt,
	    String sEndDt, StringList slUsers, boolean bExport) throws Exception {
	...
	if (bExport) {
	    ...
	    sbParamValues.append(",");
	    sbParamValues.append("Employee Dept");
	    sbParamValues.append(",");
	    sbParamValues.append("No of Days");
	    sbParamValues.append(",");
	    sbParamValues.append("Total Quantity");
	    sbParamValues.append(",");
	    sbParamValues.append("Productive Hrs");
	} else {
	...
	for (int i = 0; i < mlData.size(); i++) {
		...
		sPrdtHrs = mData.get(PRODUCTIVE_HRS);
		...
		sbParamValues.append("\",\"");
		sbParamValues.append(mData.get(DEPARTMENT_NAME));
		sbParamValues.append("\",");
		sbParamValues.append((RDMServicesUtils.isNullOrEmpty(sTotalDays) ? "0" : sTotalDays));
		sbParamValues.append(",");
		sbParamValues.append((RDMServicesUtils.isNullOrEmpty(sTotalQty) ? "0" : sTotalQty));
		sbParamValues.append(",");
		sbParamValues.append((RDMServicesUtils.isNullOrEmpty(sPrdtHrs) ? "0" : sPrdtHrs));
	    } else {
		slUsers.append(sUserName);
		sbParamValues.append(i);
	    }

	    sbParamValues.append(",");
	    sbParamValues.append((RDMServicesUtils.isNullOrEmpty(sProductivity) ? "0" : sProductivity));
	    sbParamValues.append("\n");
	}
