...
public class ExportUserTasks extends HttpServlet {
	...
	MapList mlTasks = query.searchUserTasks(sRoom, sTaskName, sTaskId, sDept, sOwner, sAssignee, sFromDate, sToDate,
		sStatus, sBatch, sStage, childTasks, parentTasks, coOwners, bDeliverables, bDeliverables, limit,
		sSearchType);
	...
	if (!bShowDeliverables) {
	    String sDelCnt = mTask.get(RDMServicesConstants.NOT_DOWNLOADED) + "("
		    + mTask.get(RDMServicesConstants.DELIVERABLE_CNT) + ")";
	    cell = row.createCell(16);
	    cell.setCellValue(sDelCnt);

	    String sChildTaskCnt = mTask.get(RDMServicesConstants.NO_CHILD_TASKS) + "("
		    + mTask.get(RDMServicesConstants.NO_CHILD_TASKS_CLOSED) + ")";
	    cell = row.createCell(17);
	    cell.setCellValue(sChildTaskCnt);
	}
    }

    private void addTaskDeliverables(HSSFSheet sheet, String sTaskName, Map<String, String> mTask,
	    boolean bShowDeliverables) throws Exception {
	Cell cell = null;
	Row row = null;
	...
	if (iSz > 0) {
		...
		cell.setCellValue((sDownloadBy == null || EMPTY.equals(sDownloadBy)) ? EMPTY
			: (mUsers.get(sDownloadBy) + " (" + sDownloadBy + ")"));
