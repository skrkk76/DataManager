	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) {
		try {
			...
			if (licensed) {
				...
				password = ((password == null || "PUSH_CONTEXT".equalsIgnoreCase(password)
						|| "RESET_CONTEXT".equalsIgnoreCase(password)) ? "" : password.trim());
				...
				if ("".equals(name)) {
					...
					} else if ("yes".equals(pushContext)
							&& (contextUser != null && RDMServicesConstants.ROLE_ADMIN.equals(contextUser.getRole()))) {
				...
				response.sendRedirect("licenseError.jsp");
			}
		} catch (Exception e) {
			e.printStackTrace();
			try {
