...
public class UserTasks extends RDMServicesConstants {
	...
	public StringList copyUserTasks(Map<String, String> mTask, String[] saTasks) throws Exception {
		...
		DataQuery query = new DataQuery();
		MapList mlTasks = query.searchUserTasks("", sTasks, "", "", "", "", "", "", "", "", "", true, true, false,
				false, false, -1, "");
	...
	public Map<String, TaskInfo> searchTasks(String sRoom, String sTask, String sDept, String sOwner, String sAssignee,
			String sFromDate, String sToDate, String sStatus, String sBatchNo, String sStage, boolean childTasks,
			boolean parentTasks, boolean coOwners, int limit, String sSearchType) throws Exception {
		Map<String, TaskInfo> mTasks = new HashMap<String, TaskInfo>();
		SimpleDateFormat sdf = new SimpleDateFormat("MM/dd/yyyy");

		DataQuery query = new DataQuery();
		MapList mlTasks = query.searchUserTasks(sRoom, "", sTask, sDept, sOwner, sAssignee, sFromDate, sToDate, sStatus,
				sBatchNo, sStage, childTasks, parentTasks, coOwners, true, true, limit, sSearchType);
	...
	public MapList searchUserTasks(String sRoom, String sTaskId, StringList slDept, String sOwner, String sAssignee,
			String sFromDate, String sToDate, String sStatus, String sBatchNo, String sStage, boolean childTasks,
			boolean parentTasks, boolean coOwners, int limit) throws Exception {
		return searchUserTasks(sRoom, sTaskId, slDept.join('|'), sOwner, sAssignee, sFromDate, sToDate, sStatus,
				sBatchNo, sStage, childTasks, parentTasks, coOwners, limit);
	}
	...
	public MapList searchUserTasks(String sRoom, String sTaskId, String sDept, String sOwner, String sAssignee,
			...
			boolean parentTasks, boolean coOwners, int limit) throws Exception {
		DataQuery query = new DataQuery();
		return query.searchUserTasks(sRoom, "", sTaskId, sDept, sOwner, sAssignee, sFromDate, sToDate, sStatus,
				sBatchNo, sStage, childTasks, parentTasks, coOwners, false, false, limit, "");
	}

	public MapList getOpenTasks(String sAssignee) throws Exception {
		DataQuery query = new DataQuery();
		return query.searchUserTasks("", "", "", "", "", sAssignee, "", "", TASK_STATUS_OPEN, "", "", true, true, false,
				true, true, -1, USER_BASED);
	}

	public MapList moveToUserTasks(String sRoom, String sTaskId, String sOwner, String sAssignee) throws Exception {
		DataQuery query = new DataQuery();
		return query.searchUserTasks(sRoom, "", sTaskId, "", sOwner, sAssignee, "", "", TASK_STATUS_WIP, "", "", true,
	...
	public Map<String, String> userTaskDetails(String sTask) throws Exception {
		...
		DataQuery query = new DataQuery();
		MapList mlTasks = query.searchUserTasks("", sTask, "", "", "", "", "", "", "", "", "", true, true, false, true,
				true, -1, "");
	...
	private MapList getTaskWBS(String sParentTask, MapList mlTaskWBS, int level, boolean deliverables)
			throws Exception {
		Map<String, String> mTask = null;
		...
		private String[] addDeliverables(String sTaskId) throws Exception {
			...
			for (int x = 0, iSz = mlTaskDels.size(); x < iSz; x++) {
				...
				while (itr.hasNext()) {
					...
					if (sAttrName.endsWith(sfxOverage)
							|| (sAttrValue == null || BLANK.equals(sAttrValue) || EMPTY.equals(sAttrValue))) {
						continue;
					}

					iTotal = 0;
					if (mDeliverableCnt.containsKey(sAttrName)) {
						iTotal = mDeliverableCnt.get(sAttrName).intValue();
					}

					iTotal = iTotal + 1;
					mDeliverableCnt.put(sAttrName, new Integer(iTotal));

					dTotal = 0.0;
					dAttrOverage = 0.0;
					if (mTaskDeliverables.containsKey(sAttrName)) {
						dTotal = mTaskDeliverables.get(sAttrName)[0].doubleValue();
						dAttrOverage = mTaskDeliverables.get(sAttrName)[1].doubleValue();
					}
					...
					mTaskDeliverables.put(sAttrName, new Double[] { new Double(dTotal), new Double(dAttrOverage) });
				}
			}
			...
			while (itr.hasNext()) {
				...
				if (mDeliverableQty.containsKey(sAttrName)) {
					...
					dAttrOverage = dAttrOverage + mDeliverableQty.get(sAttrName)[1].doubleValue();
				}
				mDeliverableQty.put(sAttrName, new Double[] { new Double(dTotal), new Double(dAttrOverage) });
