...
public class User extends RDMServicesConstants implements Serializable
{
	...
	private String cManageUsers = "";
	private String cManageAlarms = "";
	...
	{
		return ("Y".equals(cManageUsers) ? true : false);
	}
	
	public boolean manageAlarms()
	{
		return ("Y".equals(cManageAlarms) ? true : false);
	}
	...
	public int login(String sPwd) throws Exception
	{
		...
		if(mUserInfo != null && !mUserInfo.isEmpty())
		{
			...
			cManageUsers = mUserInfo.get(MANAGE_USERS);
			cManageAlarms = mUserInfo.get(MANAGE_ALARMS);
    ...
    public StringList getAllowedRoles()
    {
    	...
    	else if(ROLE_MANAGER.equals(sRole))
    	{
    		slRoles.add(ROLE_MANAGER);
    		slRoles.add(ROLE_SUPERVISOR);
    		slRoles.add(ROLE_HELPER);
    	}
    	else if(ROLE_SUPERVISOR.equals(sRole))
    	{
    		slRoles.add(ROLE_SUPERVISOR);
    		slRoles.add(ROLE_HELPER);
    	}
    	else if(ROLE_HELPER.equals(sRole))
    	{
    		slRoles.add(ROLE_HELPER);
    	}
