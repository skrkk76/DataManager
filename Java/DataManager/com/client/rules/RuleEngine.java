...
public class RuleEngine extends RDMServicesConstants {
		...
		try {
			...
			while (sRule.contains("'")) {
				...
				if (mParams.containsKey(sParam)) {
					sRule = sRule.replace(("'" + sParam + "'"), mParams.get(sParam));
				} else {
					sRule = sRule.replace(("'" + sParam + "'"), sParam);
				}
			}
			sRule = sRule.toLowerCase().replace("on", "1").replace("off", "0");
		...
		for (int i = 0; i < mlRules.size(); i++) {
			try {
				...
				while (sRule.contains("'")) {
					...
					sParam = sRule.substring(idx1, idx2).trim();
					if (mParams.containsKey(sParam)) {
						sRule = sRule.replace(("'" + sParam + "'"), mParams.get(sParam)[0]);
					} else {
						sRule = sRule.replace(("'" + sParam + "'"), sParam);
					}
				}
	...
	private boolean evaluateRule(String sRule) throws ScriptException {
		...
		for (int j = 0; j < saRuleExpr_AND.length; j++) {
			...
			for (int k = 0; k < saRuleExpr_OR.length; k++) {
				...
				// sRuleExpr3 = "(Math.round("+leftExpr+" * 10) / 10)" + sOperand +
				// "(Math.round("+rightExpr+" * 10) / 10)";
	...
	public void updateRule(String oid, String sRule, String sExec, String sRuleDesc, String sCntrlType)
			throws Exception {
		DataQuery query = new DataQuery();
