...
public class ExportUsers extends HttpServlet
{
	...
	private void addHeaders(HSSFSheet sheet)
	{
		...
		cell.setCellValue("Manage Alarms");
	...
	private void addUsers(HSSFSheet sheet, MapList mlUsers, HSSFCellStyle dateStyle, HSSFCellStyle textStyle) throws Exception
	{
		int rownum = 1;
		Row row = null;
		Cell cell = null;
		...
		for(int i=0, iSz=mlUsers.size(); i<iSz; i++)
		{
			...
			cell = row.createCell(14);
			cell.setCellValue(mUser.get(RDMServicesConstants.MANAGE_USERS));
			
			cell = row.createCell(15);
			cell.setCellValue(mUser.get(RDMServicesConstants.MANAGE_ALARMS));
			
			cell = row.createCell(16);
			cell.setCellValue(mUser.get(RDMServicesConstants.LOCALE));
