...
public class NotifyAlarms extends RDMServicesConstants
{
	...
	public void notifyUsers(Date timeStamp) throws Exception
    {
		...
		for(int i=0; i<mlNotifyAlarms.size(); i++)
		{
			...
			level1 = Integer.parseInt(mAlarm.get(NOTIFY_LEVEL1));
		    level2 = Integer.parseInt(mAlarm.get(NOTIFY_LEVEL2));
			...
			if(mNotificationAlarms.containsKey(sCntrlAlarm))
			{
				...
				firstNotify = Integer.parseInt(mNotify.get(LEVEL1_DURATION));
		        secondNotify = Integer.parseInt(mNotify.get(LEVEL2_DURATION));
				...
				if (level1 < firstNotify)
				{
					sNotifyUser = mNotify.get(NOTIFY_FIRST);
					 
					level1++;
					mAlarm.put(NOTIFY_LEVEL1, Integer.toString(level1));
					mAlarm.put(NOTIFY_LEVEL2, Integer.toString(0));
				}
				else if (level2 < secondNotify)
				{
					sNotifyUser = mNotify.get(NOTIFY_SECOND);
					 
					level2++;
					mAlarm.put(NOTIFY_LEVEL1, Integer.toString(level1));
					mAlarm.put(NOTIFY_LEVEL2, Integer.toString(level2));
				} else {
					continue;
				}
