...
<%
			StringList slWaterParams = new StringList();
			StringList slTimeParams = new StringList();
			for (Map.Entry<String, String[]> entry : mParamData.entrySet()) 
			{
				sParam = entry.getKey();
				saParamVal = entry.getValue();
				sValue = ((saParamVal == null) ? "" : saParamVal[0]);
				
				if(sParam.startsWith("room") && (sParam.endsWith("run 1") || sParam.endsWith("run 2") || sParam.endsWith("run 3") || sParam.endsWith("run 4")) 
					&& "on".equalsIgnoreCase(sValue))
				{
					String watervalve = sParam.substring(0, 7);
					watervalve = "watervalve " + (watervalve.endsWith("0") ? watervalve : watervalve.replace("0", ""));
					String time = "time" + sParam.substring(7);
					
					slWaterParams.add(sParam);
					slWaterParams.add(watervalve);
					slTimeParams.add(time);
				}
			}
			slWaterParams.sort();
			slTimeParams.sort();
