...
<%
	...
	mKeys.put("group", RDMServicesConstants.GROUP_NAME);
	...
	try
	{
			...
			while (itr.hasNext()) 
			{
				...
				if (item.isFormField())
				{
					...
					if(mKeys.containsKey(sField))
					{
						...
						{
							sValue = mUserInfo.get(mKeys.get(sField));
							sValue = ((sValue == null) ? item.getString() : (sValue + "|" + item.getString()));
							mUserInfo.put(mKeys.get(sField), sValue);
						}
						else
						{
		...
		if(!mUserInfo.containsKey(RDMServicesConstants.DEPARTMENT_NAME))
		{
			mUserInfo.put(RDMServicesConstants.DEPARTMENT_NAME, "");
		}
		
		if(!mUserInfo.containsKey(RDMServicesConstants.GROUP_NAME))
		{
			mUserInfo.put(RDMServicesConstants.GROUP_NAME, "");
		}
