...
<body>
	<form name="frm">
		<table align="center" border="0" cellpadding="1" cellspacing="0" width="<%= winWidth * 0.6 %>">
			<tr>
...
%>
			...
			<tr>
				...
				<th class="label" width="20%"><%= resourceBundle.getProperty("DataManager.DisplayText.Task_Attributes") %></th>
				<th class="label" width="20%"><%= resourceBundle.getProperty("DataManager.DisplayText.Departments") %></th>
				<th class="label" width="10%"><%= resourceBundle.getProperty("DataManager.DisplayText.Task_Alert_Duration") %></th>
				<th class="label" width="10%"><%= resourceBundle.getProperty("DataManager.DisplayText.Productivity_View") %></th>				
...
<%
			...
			for(int i=0; i<mlTasks.size(); i++)
			{
				...
				sDuration = mTask.get(RDMServicesConstants.DURATION_ALERT);
				sDuration = ("0".equals(sDuration) ? "" : sDuration);
				sTaskAttrs = mTask.get(RDMServicesConstants.TASK_ATTRIBUTES).replaceAll("\\|", "<br>");
...
%>
				<tr>
					...
					<td class="input"><%= sTaskAttrs %></td>
					<td class="input"><%= sTaskDepts %></td>
					<td class="input"><%= sDuration %></td>
					<td class="input"><%= ("true".equals(mTask.get(RDMServicesConstants.PRODUCTIVITY_TASK)) ? "Yes" : "No") %></td>
