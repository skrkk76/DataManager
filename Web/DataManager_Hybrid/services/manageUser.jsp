...
<%
	...
	if(sUserId != null && !"".equals(sUserId))
	{
		...
		sManageUsers = mUserInfo.get(RDMServicesConstants.MANAGE_USERS);
		sManageAlarms = mUserInfo.get(RDMServicesConstants.MANAGE_ALARMS);
...
%>
			...
			if(dob.value == "")
			{
				alert("<%= resourceBundle.getProperty("DataManager.DisplayText.Select_DOB") %>");
				dob.focus();
				return false;
			}
			
			var doj = document.getElementById("dateOfJoin");
			if(doj.value == "")
			{
				alert("<%= resourceBundle.getProperty("DataManager.DisplayText.Select_DOJ") %>");
				doj.focus();
				return false;
			}
...
<%
			...
			{
%>
				if(document.frm.manageUsers.checked)
				{
					document.frm.manageUsers.value = "Y";
				}
				else
				{
					document.frm.manageUsers.value = "N";
				}
				
				if(document.frm.manageAlarms.checked)
				{
					document.frm.manageAlarms.value = "Y";
				}
				else
				{
					document.frm.manageAlarms.value = "N";
				}
<%
			}
%>
		...
		function checkPassword()
		{
			...
			if(password.value == "")
			{
				alert("<%= resourceBundle.getProperty("DataManager.DisplayText.Password_Empty") %>");
				password.focus();
				return false;
			}
			return true;
...
<body onLoad="javascript:setDate('<%= sDOB %>', 'dateOfBirth');setDate('<%= sDOJ %>', 'dateOfJoin')">
	<form name="frm" method="post" action="manageUserProcess.jsp" enctype="multipart/form-data">
...
<%
					String sRole = null;
					String sUserRole = mUserInfo.get(RDMServicesConstants.ROLE_NAME);
					StringList slRoles = u.getAllowedRoles();
					for(int i=0; i<slRoles.size(); i++)
					{
						sRole = slRoles.get(i);
%>
						<option value="<%= sRole %>" <%= (!bAdd && sRole.equals(sUserRole)) ? "selected" : "" %>><%= sRole %></option>
<%
					}
%>
					</select>
...
<%
						...
						List<String> lGroups = new ArrayList<String>(mGroups.keySet());
						Collections.sort(lGroups, String.CASE_INSENSITIVE_ORDER);
						String sGroup = null;
						for(int j=0; j<lGroups.size(); j++)
						{
							sGroup = lGroups.get(j);
%>
							<option value="<%= sGroup %>" <%= slGroups.contains(sGroup) ? "Selected" : ""%>><%= sGroup %></option>
<%
						}
%>
					</select>
...
%>
				<tr>
					<td class="label"><b><%= resourceBundle.getProperty("DataManager.DisplayText.Manage") %></b></td>
					<td class="input" colspan="<%= !bAdd ? 2 : 0 %>">
						<input type="checkbox" id="manageUsers" name="manageUsers" value="Y" <%= ("Y".equals(sManageUsers) ? "checked" : "") %>><%= resourceBundle.getProperty("DataManager.DisplayText.Users") %>
						<input type="checkbox" id="manageAlarms" name="manageAlarms" value="Y" <%= ("Y".equals(sManageAlarms) ? "checked" : "") %>><%= resourceBundle.getProperty("DataManager.DisplayText.Alarms") %>
					</td>
				</tr>
<%
			}
			...
			{
%>
				<input type="hidden" name="manageUsers" id="manageUsers" value="<%= !bAdd ? sManageUsers : "N" %>">
				<input type="hidden" name="manageAlarms" id="manageAlarms" value="<%= !bAdd ? sManageAlarms : "N" %>">
<%
			}
%>
			<tr>
